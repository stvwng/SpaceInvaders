...
40 cases, 502 assertions. The suite is silent and windowless by construction — no
...
| [09](docs/09-bug-catalogue.md) | **The bug catalogue** | sixteen defect classes found here. Start here. |
...
- **Visual confirmation.** The three UI panels have now been rendered and looked
  at — which is how the clipped title, the clipped HUD and the overflowing button
  label were found, none of them by any tool. Gameplay itself still has not been
  watched end to end, and the gamepad path has never been exercised with a real
  controller attached.
